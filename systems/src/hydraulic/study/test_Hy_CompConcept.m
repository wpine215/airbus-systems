...
loopG.maxVolumeHighPressureSide=10; %Considering 10gal is the high pressure volume TODO get realistic value
...
loopG.accumulator_DeltaPressBreakpoints= [0 5 50 100 200 500 1000 10000];
loopG.accumulator_DeltaPressFlowCarac  =[0 0.005 0.01 0.02 0.08 0.15 0.35 0.5];
...
loopY.accumulator_DeltaPressBreakpoints= [0 5 50 100 200 500 1000 10000];
loopY.accumulator_DeltaPressFlowCarac  = [0 0.005 0.01 0.02 0.08 0.15 0.35 0.5];
...
function [pump,loop,aileron]= updateL(dt,pump,PTU,loop,aileron)
    ...
    %Static leaks, random formula to depend on pressure
        staticLeakVol=0.08*dt*loop.press/3000;    
    ...
    if loop.isLeft 
        if PTU.flowToLeftLoop > 0 
            ...
            actualFlow=min(loop.res,PTU.flowToLeftLoop);
        ...
        if PTU.flowToRightLoop > 0 
            ...
            actualFlow=min(loop.res,PTU.flowToRightLoop);
            loop.res=loop.res-actualFlow* dt;
        else
